...
/*
Spark streaming full outer join is not permitted
between stream & static dataset left / right outer join is permitted and stream dataset has to be on leftside for left outer
and right side for right outer.
between two stream datasets left & right outer join is permitted if -
in case of left outer join, right stream is watermarked and vice versa in case of right outer join. (i.e. left stream is watermarked)
and there is a time range constraint between right & left side events.
 */
...
object snStreamOuterJoin extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val outputQuery = joinedDF.writeStream
      .format("console")
      .outputMode("append")
      .option("checkpointLocation", "chk-point-dir1/streamOuterJoin")
